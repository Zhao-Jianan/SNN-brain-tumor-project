...
class SpikingSwinTransformerBlock3D(base.MemoryModule):
    def __init__(self, embed_dim, num_heads, window_size=(2, 2, 2), shift=False, dropout=0.1, step_mode='s'):
        super().__init__()       
        ...
        self.conv = layer.Conv3d(embed_dim, embed_dim, kernel_size=1, step_mode=step_mode)
        self.sn = neuron.LIFNode(surrogate_function=surrogate.ATan(), step_mode=step_mode)
        self.dropout = layer.Dropout(dropout, step_mode=step_mode)
...
class SpikingSwinTransformerStage3D(base.MemoryModule):
    def __init__(self, embed_dim, num_heads, window_size=(2, 2, 2), dropout=0.1, step_mode='s'):
        super().__init__()
        self.block_no_shift = SpikingSwinTransformerBlock3D(embed_dim, num_heads, window_size, shift=False, dropout=dropout, step_mode=step_mode)
        self.block_shift = SpikingSwinTransformerBlock3D(embed_dim, num_heads, window_size, shift=True, dropout=dropout, step_mode=step_mode)
...
class SpikingSwinUNet3D(base.MemoryModule):
    def __init__(self, in_channels=4, num_classes=3, embed_dim=96, num_heads=[3, 6, 12],
                 window_size=(2,2,2), dropout=0.1, T=8, step_mode='s'):
        super().__init__()
        self.T = T
        self.step_mode = step_mode

        self.patch_embed1 = SpikingPatchEmbed3D(in_channels, embed_dim, patch_size=(4,4,4), step_mode=step_mode)
        self.down_stage1 = SpikingSwinTransformerStage3D(embed_dim, num_heads[0], window_size, dropout, step_mode=step_mode)

        self.patch_embed2 = SpikingPatchEmbed3D(embed_dim, embed_dim * 2, patch_size=(2,2,2), step_mode=step_mode)
        self.down_stage2 = SpikingSwinTransformerStage3D(embed_dim * 2, num_heads[1], window_size, dropout, step_mode=step_mode)

        self.patch_embed3 = SpikingPatchEmbed3D(embed_dim * 2, embed_dim * 4, patch_size=(2,2,2), step_mode=step_mode)
        self.feature_stage = SpikingSwinTransformerStage3D(embed_dim * 4, num_heads[2], window_size, dropout, step_mode=step_mode)


        self.patch_expand2 = SpikingPatchExpand3D(embed_dim * 4, embed_dim * 2, dropout=dropout, step_mode=step_mode)
        self.up_stage2 = SpikingSwinTransformerStage3D(embed_dim * 2, num_heads[1], step_mode=step_mode)
        ...
        self.up_stage1 = SpikingSwinTransformerStage3D(embed_dim, num_heads[0], step_mode=step_mode)
        self.converge1 = SpikingAddConverge3D(embed_dim, step_mode=step_mode)


        self.final_expand = SpikingPatchExpand3D(embed_dim, embed_dim // 4, kernel_size=4, stride=4, dropout=dropout, step_mode=step_mode)

        self.readout = layer.Conv3d(embed_dim // 4, num_classes, kernel_size=1, step_mode=step_mode)
